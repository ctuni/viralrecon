...
def ivar_variants_to_vcf(FileIn,FileOut,passOnly=False,minAF=0):
    ...
    header += '#CHROM\tPOS\tID\tREF\tALT\tQUAL\tFILTER\tINFO\tFORMAT\t'+filename+'\tPVALUE\tREFCODON\tALTCODON\n'
    ...
    with open(FileIn) as f:
        for line in f:
            if not re.match("REGION",line):
                ...
                oline = CHROM+'\t'+POS+'\t'+ID+'\t'+REF+'\t'+ALT+'\t'+QUAL+'\t'+FILTER+'\t'+INFO+'\t'+FORMAT+'\t'+SAMPLE+'\t'+PVALUE+'\t'+REFCODON+'\t'+ALTCODON+'\n'
                writeLine = True
